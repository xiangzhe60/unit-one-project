...
public class Main{
	...
	double intialTemp = reader.nextDouble();
	if(intialTemp < -273.14)
	intialTemp = -273.14;
	...
	String intialPhase = "liqiud";
	if(intialTemp < 0)
	 intialPhase = "solid";
	if(intialTemp > 100)
	    intialPhase ="vapor";
	   
	   String finalPhase = "liqiud";
	if(finalTemp < 0)
        finalPhase = "solid";
	if(finalTemp > 100)
	    finalPhase ="vapor";
	...
	System.out.println("intial temperature:" + intialTemp + "C" + intialPhase);
	System.out.println("Final temperature:" + finalTemp + "C" + finalPhase);
	
	boolean endothermic = false;
	if (finalTemp > intialTemp)
	    endothermic = true;
	    
	    double heatEnergy = 0;
	    
	    if(intialPhase.equals("solid")) {
	            heatEnergy += tempChangeSolid(mass, intialTemp, finalTemp, finalPhase, endothermic);
	    ...
	    if(intialPhase.equals("liqiud")) {
	            heatEnergy += tempChangeLiquid(mass, intialTemp, finalTemp, finalPhase, endothermic);
	...
	if(intialPhase.equals("vapor")){
	    heatEnergy += tempChangeVapor(mass, intialTemp, finalTemp, finalPhase, endothermic);
...
if(endothermic)
    System.out.println("Heating (soild): " + energyChange + "kJ");
else
...
public static double tempChangeLiquid (double mass, double startTemp, double endTemp, String endPhase, boolean endothermic){
    ...
    if(endothermic)
        System.out.println("Heating (liqiud): "+ (energyChange + "kJ"));
    else 
        System.out.println("Cooling (liquid): " + energyChange + "kJ");
    return energyChange;
}

public static double fusion(double mass, boolean endotheramic) {
    double energyChange;
    if(endotheramic) {
...
public static double vaporization(double mass, boolean endotheramic) {
    double energyChange;
    if(endotheramic){
